...
function parseCurrentWeather(data){
    ...
    const { last_updated, temp_c, temp_f, condition: { text: condition_text, icon: condition_icon }, wind_mph, wind_kph, humidity, feelslike_c, feelslike_f } = data.current;
    return {
        location, 
        last_updated,
        condition_icon, 
...
async function getCurrentWeather(locationId){
    const currentWeatherUri = `${baseUri}current.json?key=${apiKey}&q=id:${locationId}`;
